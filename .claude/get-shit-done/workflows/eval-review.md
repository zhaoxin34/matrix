...
```bash
AUDITOR_MODEL=$(node "/Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-eval-auditor --raw)
```
...
```markdown
Read /Volumes/data/working/ai/matrix/.claude/agents/gsd-eval-auditor.md for instructions.
