...
```bash
INIT=$(node "/Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "/Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
