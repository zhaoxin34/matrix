...
- **Global scope**: `/Volumes/data/working/ai/matrix/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Volumes/data/working/ai/matrix/.claude/notes/`)
...
2. Glob `/Volumes/data/working/ai/matrix/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Volumes/data/working/ai/matrix/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Volumes/data/working/ai/matrix/.claude/notes/` — works in any directory
