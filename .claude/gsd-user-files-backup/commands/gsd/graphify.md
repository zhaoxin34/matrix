...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node /Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node /Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node /Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node /Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "/Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs" graphify build)
```
...
4. **Write diff snapshot:**
   ```
   node \"/Volumes/data/working/ai/matrix/.claude/get-shit-done/bin/gsd-tools.cjs\" graphify build snapshot
   ```
