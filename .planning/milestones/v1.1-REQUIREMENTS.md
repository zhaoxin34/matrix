...
## v1.0 Requirements (Completed in prior milestone)
...
## v1.1 Requirements (Active) — ALL COMPLETE

### 1. Authentication (Phase 4)

#### User Registration

- [x] **AUTH-01**: User can register with phone number and password
...
- [x] **AUTH-02**: User receives verification code via SMS for phone verification
...
- [x] **AUTH-03**: User can login with phone number and password
  - Return JWT access token (30 min expiry) + refresh token (7 days)
  - Failed login tracking (5 attempts = 15 min lockout)
  - Account lockout notification

- [x] **AUTH-04**: JWT access token refresh mechanism
...
- [x] **CAT-01**: Two-level category hierarchy (L1 → L2)
...
- [x] **PROD-03**: Search products by keyword
  - Database ILIKE search on product name
  - Minimum 2 characters to trigger search
  - Maximum 50 results per query

- [x] **PROD-04**: Search autocomplete/suggestions
...
|----------|---------|------------|
| SMS provider for verification | Aliyun SMS / Tencent SMS / Mock for demo | **Mocked for v1.1** (prints to console) |
| Image storage | Cloud storage (S3/R2) / Local / External URLs | **External URLs** for demo |
...
- Custom JWT auth over fastapi-users for AI agent observability
- SMS verification mocked; real SMS deferred to v1.2
- Rate limiting via slowapi library

### Phase 5 Key Decisions
- L1/L2 category hierarchy for clean UX + AI simulation simplicity
- Ant Design 5 for fast development, consistent UI
- Zustand for state management (lightweight, TypeScript-friendly)
- AutoComplete for search with 300ms debounce

---

*Requirements archived: 2026-04-16*
