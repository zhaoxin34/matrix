...
- **THEN** the system MUST persist a row in `knlg_rule` with `workspace_id = W.id`, `created_by = current_user.id`, `status = 'draft'`, `version = '1.0'`, `confidence = payload.confidence or 0.5`
