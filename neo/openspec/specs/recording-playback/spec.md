...
### Requirement: Show segment comments alongside playback

The system SHALL surface annotations (recording-segment-comments) attached to the playing segment during playback.

#### Scenario: Active comment bubble on canvas

- **WHEN** the current playback time `t` is within a comment's `[show_time, hide_time]` range
- **THEN** the system shows the comment as a bubble overlay on the playback canvas

#### Scenario: Multiple active comments stack

- **WHEN** N >= 2 comments are simultaneously active
- **THEN** the system stacks their bubbles vertically with an N/M counter and up/down navigation

#### Scenario: Side-panel comment list highlights active entry

- **WHEN** a comment is currently active
- **THEN** the system highlights its entry in the side-panel comment list

#### Scenario: Comment timeline markers on progress bar

- **WHEN** the playing segment has N comments
- **THEN** the system overlays N colored range markers above the progress bar at their respective `[show_time, hide_time]` positions

#### Scenario: Side-panel hover highlights canvas bubble

- **WHEN** the user hovers a side-panel comment entry
- **THEN** the system highlights the corresponding canvas bubble without changing playback position

#### Scenario: Jump to comment via side panel

- **WHEN** the user clicks the jump control on a side-panel comment entry
- **THEN** the system seeks playback to that comment's `show_time` and starts playback
