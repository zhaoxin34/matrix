...
export class SessionManager {
   ...
   */
  createSession(connection: WsConnection, options: {
    cwd?: string;
    modelId?: string;
    ...
    this.log.debug(`Session created: ${sessionId} for connection ${connection.id}`);
   ...
   */
  getConnectionSession(connection: WsConnection): { sessionId: string; session: IAgentSession } | undefined {
...
async function handleSessionSend(
  ...
  const params = request.params as { prompt?: string; images?: unknown[] } | undefined;

  if (!params?.prompt) {
    return createError(JsonRpcErrorCode.InvalidParams, 'Missing required parameter: prompt', request.id);
  }
  ...
  try {
    ...
    session.prompt(params.prompt).catch((err) => {
      context.connection.send(createError(JsonRpcErrorCode.InternalError, String(err), null));
    });
...
async function handleSessionSteer(
  ...
  if (!params?.text) {
    return createError(JsonRpcErrorCode.InvalidParams, 'Missing required parameter: text', request.id);
  }

  const { session } = context;

  if (!session.isStreaming) {
    return createError(-32002, 'Session is not currently streaming', request.id);
  }
...
const methodHandlers: Record<string, (request: JsonRpcRequest, context: ApiContext) => Promise<JsonRpcResponse | JsonRpcErrorResponse>> = {
...
export async function handleRequest(
  ...
  if (!Array.isArray(batchResult) && 'error' in batchResult) {
    ...
    if (!parseResult.valid) {
      return [createError(JsonRpcErrorCode.ParseError, parseResult.error.message, null)];
    }
...
async function handleParsedMessage(
  ...
  if (isRequest(message)) {
    ...
    if (!handler) {
      results.push(createError(JsonRpcErrorCode.MethodNotFound, `Method not found: ${request.method}`, request.id));
      return results;
    }
    ...
    try {
      ...
      results.push(createError(JsonRpcErrorCode.InternalError, String(err), request.id));
    }
  }
