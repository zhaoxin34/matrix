...
export interface WorkerPool {
	...
	getStats(): PoolStats;
	on(
		event: "worker:ready" | "worker:error" | "worker:event",
	...
	const pool: WorkerPool = {
		...
		async acquire(sessionId: string): Promise<number> {
			...
			return new Promise<number>((resolve, reject) => {
				...
				log.debug(
					`Session ${sessionId} queued. ${waitingQueue.length} waiting.`,
				);
			});
		},
		...
		release(workerId: number): void {
			...
			if (waitingQueue.length > 0) {
				...
				log.debug(
					`Worker ${workerId} assigned to queued session ${waiting.sessionId}`,
				);
			}
		},
 ...
 */
export async function createRealWorkerPool(
	options: PoolOptions = {},
): Promise<WorkerPool> {
