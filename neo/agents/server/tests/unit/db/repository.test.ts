...
import { createTables, dropTables, tableNames } from '../../../src/db/schema.js';
...
describe('Message Repository', () => {
  ...
  describe('getBySessionId()', () => {
    it('should return messages ordered by created_at', async () => {
      await sessionRepo.create({ id: 'session-1' });
      await messageRepo.create({ sessionId: 'session-1', role: 'user', content: 'First' });
      await messageRepo.create({ sessionId: 'session-1', role: 'assistant', content: 'Second' });
      await messageRepo.create({ sessionId: 'session-1', role: 'user', content: 'Third' });
  ...
  describe('getCountBySessionId()', () => {
    it('should return correct count', async () => {
      await sessionRepo.create({ id: 'session-1' });
      await messageRepo.create({ sessionId: 'session-1', role: 'user', content: 'A' });
      await messageRepo.create({ sessionId: 'session-1', role: 'assistant', content: 'B' });
  ...
  describe('deleteBySessionId()', () => {
    it('should delete all messages for session', async () => {
      await sessionRepo.create({ id: 'session-1' });
      await messageRepo.create({ sessionId: 'session-1', role: 'user', content: 'A' });
      await messageRepo.create({ sessionId: 'session-1', role: 'user', content: 'B' });
