...
function DownloadLink({ filePath, label = "Download" }: { filePath: string; label?: string }) {
  ...
  return (
    <a
      href={apiUrl(`/api/files/${encoded}?type=read`)}
...
function ImageViewer({ filePath, cwd }: { filePath: string; cwd?: string }) {
  ...
  useEffect(() => {
    ...
    const encoded = encodeFilePathForApi(filePath);
    const es = new EventSource(apiUrl(`/api/files/${encoded}?type=watch`));
    esRef.current = es;
  ...
  const encoded = encodeFilePathForApi(filePath);
  const src = apiUrl(`/api/files/${encoded}?type=read${bust ? `&v=${bust}` : ""}`);
...
function DocumentViewer({ filePath, cwd }: { filePath: string; cwd?: string }) {
  ...
  const previewUrl = isPdf
    ? apiUrl(`/api/files/${encoded}?type=read${bust ? `&v=${bust}` : ""}`)
    : apiUrl(`/api/files/${encoded}?type=preview${bust ? `&v=${bust}` : ""}`);
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/files/${encoded}?type=meta`))
      .then((r) => r.json())
...
function TextFileViewer({ filePath, cwd }: Props) {
  ...
  const fetchContent = useCallback((filePath: string, isRefresh = false) => {
    const encoded = encodeFilePathForApi(filePath);
    return fetch(apiUrl(`/api/files/${encoded}?type=read`))
      .then((r) => r.json())
