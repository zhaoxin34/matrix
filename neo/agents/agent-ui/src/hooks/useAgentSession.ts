...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false) => {
    try {
      ...
      const url = includeState
        ? apiUrl(`/api/sessions/${encodeURIComponent(sid)}?includeState`)
        : apiUrl(`/api/sessions/${encodeURIComponent(sid)}`);
  ...
  const loadContext = useCallback(async (sid: string, leafId: string | null) => {
    try {
      const url = leafId
        ? apiUrl(`/api/sessions/${encodeURIComponent(sid)}/context?leafId=${encodeURIComponent(leafId)}`)
        : apiUrl(`/api/sessions/${encodeURIComponent(sid)}/context`);
      const res = await fetch(url);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  // Load model list
  useEffect(() => {
    fetch(apiUrl("/api/models")).then((r) => r.json()).then((d: { models: Record<string, string>; modelList?: { id: string; name: string; provider: string }[]; defaultModel?: { provider: string; modelId: string } | null; thinkingLevels?: Record<string, string[]>; thinkingLevelMaps?: Record<string, Record<string, string | null>> }) => {
