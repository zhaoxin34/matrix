...
| 后端 Event 字段 | 来源 |
|----------------|------|
| `name` | `interceptor.eventName`(规则里硬编码,或在 trigger 时动态推断) |
| `entity_name` | `entity_extractor` 运行时求值(从 DOM XPath 或网络 URL 提取) |
| `target_entity_name` | 同上(可选) |
...
| 后端 Status 字段 | 来源 |
|----------------|------|
| `entity_name` | `action.config.entityName`(规则里配置) |
...
|--------|------|---------------|-------------|---------|
| `collect_event` | after | ❌ | ✅ fire-forget | `eventName`, `entity_name`, `actor` |
| `collect_status` | before/after | ❌ | ✅ fire-forget | `entityName`, `attributes`(XPath map) |
