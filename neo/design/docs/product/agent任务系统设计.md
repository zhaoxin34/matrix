...
Agent拥有者在[Agent嵌入](/docs/product/agent-ingest)目标系统中，添加临时任务到任务列表中, 按照优先级排序执行。

### 故事二

Agent拥有者在[workspace](/docs/product/workspace)查看所有任务，包括他自己的和别人的，对于自己的任务，可以有一些非只读的操作。
