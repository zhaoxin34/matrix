...
- Agent拥有者通过[Agent嵌入](./agent-ingest) 临时创建的任务
- Agent拥有者创建的周期任务
- 其他用户分配的派发任务
- **Agent 主动创建的任务**（Agent 可根据学习到的模式自动创建，创建的任务归属于 Agent 的 owner）

### 任务属性

- 创建人 (owner_id): 用户id，任务的创建者和拥有者
- workspaceId: 工作区id。Agent 必须属于此 workspace。查看[workspace设计](./workspace)
...
| 假设                  | 说明                                                            |
| --------------------- | --------------------------------------------------------------- |
| 任务归属于用户        | 任务的真正归属者是创建它的用户，Agent 只是执行者                |
| Agent类型决定任务类型 | 不同类型的Agent（学习型、引导型、主动型）可能支持不同的任务类型 |
| 任务执行环境隔离      | 任务在独立环境中执行，避免影响目标系统的正常运行                |

#### 任务执行约束

| 约束           | 说明                                         |
| -------------- | -------------------------------------------- |
| 最大执行时长   | 单次任务执行最大时长为30分钟，超过后自动终止 |
| 任务队列限制   | 单个Agent最多同时执行3个任务                 |
| 重试冷却时间   | 任务失败后需等待至少1分钟才能重试            |
| 任务优先级冲突 | 当优先级相同时，按创建时间先进先出           |

#### 周期任务约束

| 约束       | 说明                                  |
| ---------- | ------------------------------------- |
| Cron表达式 | 支持标准5位Cron表达式，不支持秒级触发 |
| 最小间隔   | 周期任务最小间隔为5分钟               |
| 时区处理   | 统一使用服务器时区执行                |
| 冲突处理   | 同一时刻只能有一个周期任务实例在执行  |

#### 任务执行录像

| 约束     | 说明                              |
| -------- | --------------------------------- |
| 录像格式 | 使用rrweb录制，格式为.webm        |
| 录像时长 | 单次录像最长2小时，超出后分段存储 |
| 录像存储 | 录像保留30天后自动清理（可配置）  |
| 录像权限 | 只有Agent拥有者和系统管理员可查看 |

#### Webhook反馈约束

| 约束     | 说明                                           |
| -------- | ---------------------------------------------- |
| 超时设置 | Webhook回调超时时间为10秒                      |
| 重试策略 | 失败后最多重试3次，间隔分别为1min、5min、15min |
| 签名验证 | 支持HMAC-SHA256签名验证                        |
| 幂等性   | 同一任务执行结果可能多次发送，接收方需保证幂等 |

#### 安全约束

| 约束         | 说明                                      |
| ------------ | ----------------------------------------- |
| 敏感操作确认 | 涉及删除、数据修改等敏感操作需二次确认    |
| 操作日志     | 所有任务执行操作均记录日志                |
| 权限边界     | 用户只能操作自己拥有或被授权的Agent的任务 |
...
> 注："新手员工、资深员工、一般员工"只是对 Agent 拥有者的描述性分类，无实际业务意义。

### 故事一 在Agent嵌入 系统中添加、查看、管理任务

Agent拥有者在[Agent嵌入](./agent-ingest)目标系统中，添加临时任务到任务列表中, 按照优先级排序执行。
