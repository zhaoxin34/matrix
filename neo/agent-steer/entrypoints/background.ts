...
export default defineBackground(() => {
  ...
  browser.webNavigation.onCommitted.addListener(async (details) => {
    ...
    console.log('[background] nav committed:', details.url, 'tab', details.tabId)
    try {
  ...
  browser.runtime.onMessage.addListener(
    (message: { type: string; payload?: unknown }, _sender: unknown) => {
      if (message?.type === 'rrweb-event') {
        const ev = message.payload as { type?: number; timestamp?: number } | undefined
        if (ev?.type === 4) return // Meta 事件少打
        console.log('[background] rrweb event type=' + ev?.type + ' t=' + ev?.timestamp)
      }
    },
  )
})
