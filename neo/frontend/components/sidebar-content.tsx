...
const baseMenuGroups: MenuGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        title: "Auth Bridge 测试",
        url: "/auth-bridge/test",
        icon: TestTube,
      },
    ],
  },
...
function useWorkspaceMenuItems(): { items: MenuItem[]; showPrompt: boolean } {
  ...
  // 优先使用 URL 中的 workspace code（避免 store 与 URL 不一致时链接错误）
  const urlMatch = pathname.match(/^\/workspace\/([^/]+)/);
  const urlWorkspaceCode = urlMatch ? urlMatch[1] : null;
  const activeWorkspaceCode = urlWorkspaceCode || currentWorkspace?.code;

  // 未选择工作区时显示提示
  if (!activeWorkspaceCode) {
  ...
  return {
    items: [
      {
        title: "嵌入网站管理",
        url: `/workspace/${activeWorkspaceCode}/embedded-sites`,
        icon: Globe,
      },
      {
        title: "Agent 管理",
        url: `/workspace/${activeWorkspaceCode}/agents`,
        icon: Bot,
      },
      {
        title: "任务管理",
        url: `/workspace/${activeWorkspaceCode}/tasks`,
        icon: ListTodo,
      },
      {
        title: "录像管理",
        url: `/workspace/${activeWorkspaceCode}/recordings`,
        icon: Video,
      },
      {
        title: "Agent Steer 演示",
        url: `/workspace/${activeWorkspaceCode}/agent-steer-demo`,
        icon: Video,
      },
      {
        title: "事件",
        url: `/workspace/${activeWorkspaceCode}/events`,
        icon: Activity,
      },
      {
        title: "状态",
        url: `/workspace/${activeWorkspaceCode}/status`,
        icon: Database,
      },
      {
        title: "拦截器",
        url: `/workspace/${activeWorkspaceCode}/interceptors`,
        icon: Layers,
      },
      {
        title: "知识库与问答库",
        url: `/workspace/${activeWorkspaceCode}/knlg-base`,
        icon: Brain,
      },
    ],
    showPrompt: false,
