...
export function StatusList({ workspaceCode, className }: StatusListProps) {
  ...
  const fetchStatuses = useCallback(
    async (entityType: string, entityId: string, pageNum: number) => {
      setLoading(true);
      try {
  ...
  }, [workspaceCode, pageSize, entityTypeFilter, entityIdSearch]);

  const handlePageSizeChange = useCallback(
    (newSize: number) => {
      setPageSize(newSize);
      setPage(1);
      fetchStatuses(entityTypeFilter, entityIdSearch, 1);
    },
    [fetchStatuses, entityTypeFilter, entityIdSearch],
  );
  ...
  const handleEntityTypeFilter = useCallback(
    (entityType: string) => {
      ...
      setPage(1);
      fetchStatuses(entityType, entityIdSearch, 1);
    },
    [fetchStatuses, entityIdSearch],
  );

  const handleEntityIdSearch = useCallback(
    (entityId: string) => {
      setEntityIdSearch(entityId);
      fetchStatuses(entityTypeFilter, entityId, 1);
    },
    [fetchStatuses, entityTypeFilter],
  );

  const handlePageChange = useCallback(
    (newPage: number) => {
      fetchStatuses(entityTypeFilter, entityIdSearch, newPage);
    },
    [fetchStatuses, entityTypeFilter, entityIdSearch],
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("状态已删除");
      setDeleteId(null);
      fetchStatuses(entityTypeFilter, entityIdSearch, page);
    } catch (error) {
      const err = error as { code?: number };
      toast.error(getErrorMessage(err.code || 9001));
    } finally {
    ...
    <div className={`space-y-4 ${className ?? ""}`}>
      ...
      <div className="flex items-center justify-between gap-4">
        <StatusHeader
          ...
          onEntityIdSearch={handleEntityIdSearch}
          currentEntityType={entityTypeFilter}
          currentEntityId={entityIdSearch}
        ...
        <>
          <Card>
            ...
            <div className="flex items-center px-4 py-3 border-b bg-muted/50 text-sm font-medium text-muted-foreground">
              <div className="flex-1 min-w-0">实体</div>
              <div className="w-32 hidden md:block">来源</div>
              <div className="w-40 hidden lg:block">统计时间</div>
              ...
              {statuses.map((status) => (
                ...
                >
                  ...
                  <div className="w-40 hidden lg:block text-xs text-muted-foreground">
                    {formatTimestamp(status.stat_at)}
                  </div>
