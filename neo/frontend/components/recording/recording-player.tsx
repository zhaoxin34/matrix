...
export function RecordingPlayer({
	...
	// This avoids a two-step "seek to start, then RAF-seek again" pattern
	// that was both fragile (race against rrweb's async state) and
	// visually jarring (the user would briefly see the segment start
	// before the canvas jumped to the comment's show_time).
	const playFromSegment = useCallback(
		(index: number, seekOffsetMs = 0) => {
			...
			// Single seek; rrweb internally does pause+play(time).
	...
	const handleJumpComment = useCallback(
		(comment: SegmentComment) => {
			...
			if (idx < 0) return;
			// Single seek: jump straight to (segment start + show_time) and
			// begin playback. No RAF, no double-seek — rrweb handles the
			// pause+play internally inside ReplayerController.seek().
