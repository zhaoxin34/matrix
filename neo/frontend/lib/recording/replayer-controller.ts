...
export class ReplayerController {
	...
	play(): void {
		...
		// rrweb's Replayer.play(timeOffset = 0) defaults to 0, which would
		// reset currentTime to the start of the recording — that's the
		// opposite of what "resume from current position" should do.
		// Pass the current time explicitly so playback resumes in place.
