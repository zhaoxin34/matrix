	...
	const onSubmit = async (data: LoginFormData) => {
		...
		try {
			...
			if (response.code === 0) {
				...
				// Get user's workspaces and navigate to the first one
				try {
					const workspaces = await getMyWorkspaces();
					if (workspaces.list.length > 0) {
						const firstWorkspace = workspaces.list[0];
						router.push(`/workspace/${firstWorkspace.code}/embedded-sites`);
					} else {
						// No workspace, redirect to workspace creation
						router.push("/workspace/new");
					}
				} catch {
					// If getting workspaces fails, use default redirect
			...
			<div className="flex flex-1 items-center justify-center p-8">
				<div className="w-full max-w-md space-y-8">
					...
					>
						...
						<Label
							htmlFor="sms-login"
							className="flex cursor-pointer items-center justify-center gap-2 rounded-md px-3 py-2 text-sm transition-colors data-[state=unchecked]:hover:bg-muted"
						>
