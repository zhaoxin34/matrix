...
export default function StatsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const token = JSON.parse(localStorage.getItem("neo-auth") || "{}")?.state?.user?.token;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">问题数</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">访谈数</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">问答数</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">已回答率</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <ul className="space-y-2">
                {stats.top_domains.map((d) => (
                  <li key={d.domain} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">{d.question_count} 问题</span>
                  </li>
                ))}
              </ul>
            )}
