            ...
            {selectedTreeId && (
              <div className="mt-4">
                <Label className="text-xs text-muted-foreground">
                  预览问题 ({questionTrees.find((t) => t.id === selectedTreeId)?.questions.length ?? 0} 个)
                </Label>
                <div className="mt-2 space-y-2">
                  {(questionTrees.find((t) => t.id === selectedTreeId)?.questions ?? []).slice(0, 3).map((q, i) => (
                    <div
                      key={q.id || i}
                  ...
                  {(questionTrees.find((t) => t.id === selectedTreeId)?.questions.length ?? 0) > 3 && (
                    <div className="text-xs text-muted-foreground text-center">
                      ... 还有 {(questionTrees.find((t) => t.id === selectedTreeId)?.questions.length ?? 0) - 3} 个问题
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>专家: {MOCK_EXPERTS.find((e) => e.id === selectedExpertId)?.name}</span>
              {interviewId && (
                <>
                  <span>·</span>
