...
export async function GET(
	...
	try {
		...
		// Forward query params - use workspace_code directly (supports both ID and code)
		const queryString = request.nextUrl.searchParams.toString();
		const url = `${API_BASE_URL}/api/v1/workspaces/${workspace_code}/members${queryString ? `?${queryString}` : ""}`;
...
export async function POST(
	...
	try {
		...
		const url = `${API_BASE_URL}/api/v1/workspaces/${workspace_code}/members`;
		const response = await fetch(url, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
