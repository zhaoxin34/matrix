...
class TestDecodeToken:
    ...
    def test_decode_token_with_wrong_secret_returns_none(self):
        ...
        try:
            ...
            token = jwt.encode({"sub": "1", "exp": 9999999999}, "different-secret-key", algorithm="HS256")
            payload = decode_token(token)
    ...
    def test_decode_token_without_expiration(self):
        ...
        token = jwt.encode({"sub": "1", "iat": 9999999999}, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
        payload = decode_token(token)
