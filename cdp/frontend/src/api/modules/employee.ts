...
export const employeeApi = {
  list: async (params: EmployeeListParams = {}): Promise<EmployeeListResponse> => {
    const res = await apiClient.get<ApiResponse<EmployeeListResponse>>('/employees', { params })
  ...
  bindUser: async (id: number, userId: number): Promise<Employee> => {
    const res = await apiClient.post<ApiResponse<Employee>>(`/employees/${id}/bind-user`, {
      user_id: userId,
    })
    return res.data.data!
  },
  ...
  initiateTransfer: async (id: number, data: EmployeeTransferCreate): Promise<EmployeeTransfer> => {
  ...
  approveTransfer: async (employeeId: number, transferId: number): Promise<Employee> => {
  ...
  importExcel: async (file: File): Promise<{ success: number; errors: unknown[] }> => {
    const formData = new FormData()
    formData.append('file', file)
    const res = await apiClient.post<ApiResponse<{ success: number; errors: unknown[] }>>(
      '/employees/import',
      formData,
