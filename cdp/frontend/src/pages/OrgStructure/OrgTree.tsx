  ...
  const renderTitle = (node: OrgUnitTreeNode): React.ReactNode => {
    const menuItems = [
      {
        ...
        "data-testid": `menu-org-add-child-${node.id}`,
      },
      {
        key: "edit",
        icon: <EditOutlined />,
        label: "编辑",
        onClick: () => onEdit(node),
        "data-testid": `menu-org-edit-${node.id}`,
      },
      {
        key: "toggle",
        ...
        "data-testid": `menu-org-toggle-${node.id}`,
      },
      { type: "divider" as const },
      {
        ...
        "data-testid": `menu-org-delete-${node.id}`,
      },
    ];
    ...
    return (
      ...
      >
        <span
          style={{
            color: node.status === "inactive" ? "#999" : undefined,
            cursor: "pointer",
          }}
        ...
        <Tooltip
          title={`直属: ${node.member_count} | 含下级: ${node.total_member_count}`}
        >
          <span style={{ fontSize: 12, color: "#999" }}>
            ({node.total_member_count})
          </span>
        </Tooltip>
