...
const mockRoles: ProjectRole[] = [
  { id: 1, name: "项目管理员", code: "admin", description: "项目全部权限", member_count: 2, created_at: "2024-01-15T10:00:00Z" },
  { id: 2, name: "开发者", code: "developer", description: "开发权限", member_count: 5, created_at: "2024-01-15T10:00:00Z" },
  { id: 3, name: "访客", code: "viewer", description: "只读权限", member_count: 3, created_at: "2024-01-15T10:00:00Z" },
];
  ...
  const handleModalOk = () => {
    ...
    if (modalMode === "create") {
      ...
      setRoles((prev) =>
        prev.map((r) =>
          r.id === editingRole.id
            ? { ...r, name: formData.name, code: formData.code, description: formData.description }
            : r
        )
      );
      alert("更新成功");
    ...
    <Box sx={{ p: 3 }}>
      <Card>
        <Box sx={{ p: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button variant="contained" onClick={handleAdd} data-testid="btn-create-role">
            新建角色
          </Button>
        </Box>
                ...
                <TableRow key={role.id} hover>
                  ...
                  <TableCell>
                    <Box sx={{ display: "flex", gap: 0.5 }}>
                      <Button size="small" onClick={() => handleEdit(role)} data-testid={`btn-edit-role-${role.id}`}>
                        编辑
                      </Button>
                      <Button size="small" color="error" onClick={() => handleDelete(role.id)} data-testid={`btn-delete-role-${role.id}`}>
                        删除
                      </Button>
                    </Box>
                  </TableCell>
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{modalMode === "create" ? "新建角色" : "编辑角色"}</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            label="角色名称"
