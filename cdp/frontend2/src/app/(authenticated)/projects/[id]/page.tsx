...
import { projectApi, Project, ProjectMember, OrgProject } from "@/lib/projectApi";
...
export default function ProjectDetailPage() {
  ...
  const handleUpdateMemberRole = async (userId: number, role: string) => {
    try {
      await projectApi.updateMemberRole(Number(id), userId, role as "admin" | "member");
      alert("更新角色成功");
      fetchMembers();
    } catch {
      alert("更新角色失败");
  ...
  const getRoleColor = (role: string) => (role === "admin" ? "primary" : "default");
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">{project?.name || "加载中..."}</Typography>
      </Breadcrumbs>
      ...
      <Card>
        ...
        <TabPanel value={tabValue} index={0}>
          <Box sx={{ mb: 2 }}>
            <Button variant="contained" onClick={() => setMemberModalOpen(true)} data-testid="btn-add-member">
              添加成员
            </Button>
          </Box>
          <TableContainer>
              ...
              <TableBody>
                {members.map((member) => (
                  <TableRow key={member.id}>
                    ...
                    <TableCell>
                      <Chip label={member.role} color={getRoleColor(member.role) as "primary" | "default"} size="small" />
                    </TableCell>
        ...
        <TabPanel value={tabValue} index={1}>
          <Box sx={{ mb: 2 }}>
            <Button variant="contained" onClick={() => setOrgModalOpen(true)} data-testid="btn-add-org">
              关联组织
            </Button>
          </Box>
          <TableContainer>
              ...
              <TableBody>
                {orgs.map((org) => (
                  <TableRow key={org.id}>
                    ...
                    <TableCell>
                      <IconButton color="error" size="small" onClick={() => handleRemoveOrg(org.org_id)}>
                        🗑️
                      </IconButton>
                    </TableCell>
                  </TableRow>
      ...
      {/* Add Member Dialog */}
      <Dialog open={memberModalOpen} onClose={() => setMemberModalOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddMember} variant="contained">确定</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={orgModalOpen} onClose={() => setOrgModalOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) => setOrgFormData({ ...orgFormData, org_id: e.target.value })}
            margin="normal"
            required
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddOrg} variant="contained">确定</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
