...
import { projectApi, Project, ProjectCreate, ProjectUpdate } from "@/lib/projectApi";
...
export default function ProjectListPage() {
  ...
  const filteredProjects = projects.filter(
    (p) =>
      p.name.includes(searchKeyword) || p.code.includes(searchKeyword)
  );
  ...
  return (
    <Box sx={{ p: 3, height: "calc(100vh - 64px)", display: "flex", flexDirection: "column" }}>
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{modalMode === "create" ? "新建项目" : "编辑项目"}</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            label="项目名称"
