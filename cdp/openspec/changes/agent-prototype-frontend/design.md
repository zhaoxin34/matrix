...
- 原型 CRUD（列表/创建/编辑/删除）
- Prompts 管理（6 种类型，Markdown 编辑，直接存储在原型中）
- 版本发布与回滚
...
- 版本历史 Tab 支持发布和回滚操作
- 创建/编辑表单包含 name/description/model/temperature/max_tokens 和 prompts
...
/agent-prototypes/new            → 创建页
/agent-prototypes/[id]            → 详情页（3 个 Tab）
/agent-prototypes/[id]/edit       → 编辑页
```
...
Prompts 直接存储在原型表中，前端通过 `prompts` JSON 字段管理。
...
**Rationale**: 与现有 projectApi.ts 保持一致的模式。prompts 作为原型字段的一部分，无需独立 API。
...
原型支持 enabled ↔ disabled 状态切换，在详情页提供切换按钮。
...
| AgentPrototypePrompts | `components/agent-prototype/AgentPrototypePrompts.tsx` | Prompts 管理（6个类型 Tab） |
