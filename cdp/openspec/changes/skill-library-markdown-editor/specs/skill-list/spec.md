...
系统 SHALL 支持按 status 筛选技能。status 参数支持单个值或逗号分隔的多个值。

#### Scenario: Filter by single status
- **WHEN** 用户发送 GET `/api/v1/skills?status=active` 请求
- **THEN** 系统只返回 status = 'active' 的技能
...
- **WHEN** 用户发送 GET `/api/v1/skills?status=invalid` 请求
- **THEN** 系统返回 422 Unprocessable Entity，错误信息说明 status 无效

### Requirement: Combined filters
系统 SHALL 支持多个筛选条件组合。

#### Scenario: Combine level and status filters
- **WHEN** 用户发送 GET `/api/v1/skills?level=Planning&status=active` 请求
- **THEN** 系统返回同时满足 level='Planning' 和 status='active' 的技能

### Requirement: Default list behavior
系统 SHALL 默认只返回未删除且 status 为 active 的技能（不包含 draft）。

#### Scenario: List excludes deleted skills by default
- **WHEN** 用户发送 GET `/api/v1/skills` 请求
- **THEN** 系统不返回 deleted_at 不为 NULL 的技能

#### Scenario: List excludes draft skills by default
- **WHEN** 用户发送 GET `/api/v1/skills` 请求
- **THEN** 系统不返回 status = 'draft' 的技能
